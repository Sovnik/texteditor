...
void save() {
	...
	if (fd != -1){
		if (ftruncate(fd, len) != 1) {
			if (write(fd, buf, len) == len) {
				...
				free(buf);
				E.dirty = 0;
				setStatusMessage("%d bytes written", len);
				return;
			}
		}
		close(fd);
	...
	free(buf);
	setStatusMessage("IO Error %s", strerror(errno));
}
		...
		if (filerow >= E.numrows) {
			if (E.numrows == 0 && y == E.screenrows / 3) {
				...
				int welcomelen = snprintf(welcome, sizeof(welcome),
					"Party time editor -- version %s", VERSION);
	...
	switch (c) {
		...
		case CTRL_KEY('q'):
			if (E.dirty && quit_times > 0) {
				setStatusMessage("File has unsaved changed. Press Ctrl-Q %d times to force quit", quit_times);
				quit_times--;
				return;
			}
			write(STDOUT_FILENO, "\x1b[2J", 4);
